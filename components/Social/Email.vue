...
const {url, btnText, hasIcon, subject, customIcon} = withDefaults(defineProps<{
  url: string,
  btnText: string,
  hasIcon: boolean,
  subject: string,
...
const openShareWindow = () => {
  emit('onShare', { name: 'Email' });
  const mailUrl = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(url)}`;
